...
class TestForm(BaseTest):
    ...
    def test_form_submit_missing_mandatory_fields(self):
        ...
        for locator, msg in validation_messages:
            validation_msg = page_nav.find_element(self.driver, *locator)
            check.is_true(validation_msg, msg)
            if validation_msg:
                check.equal(validation_msg.text, TestData.missing_mandatory_fields_msg, msg)